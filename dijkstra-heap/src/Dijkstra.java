...
public class Dijkstra {
	 ...
	 * indica, para cada vertice k, cual es el vertice anterior a k en el camino mínimo desde s hasta k. 
	 */
	public static Par<int[], int[]> dijkstra(Grafo g, int s) {
		...
		d[s] = 0; p[s] = indefinido;	// inicializacion para el nodo origen
		
		for (k = 0; k < n; k++) {			// inicializacion para los demás nodos
			if (k != s) {
				d[k] = coste(g, s, k);
				p[k] = s;
				Q.insertar(k, d[k]);
	...
	private static void crearOpciones(Options options) {
		...
		options.addOption("n", true, "cantidad de vertices en el grafo");
